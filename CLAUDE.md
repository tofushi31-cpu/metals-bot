...
- `signals.py` — `METALS`, `fetch_prices` (yfinance, с ретраями, пустой ответ = ошибка; кэш в памяти на `CACHE_TTL_MINUTES` минут по инструменту+таймфрейму; замок — параллельные yf.download смешивают ответы), `TIMEFRAMES`/`fetch_timeframe` (15м–неделя, 4ч склеивается из часовых, обрезка до `CHART_CANDLES`), `compute_fib_zones` (уровни Фибоначчи по свингу 60 свечей), `compute_indicators` (RSI/ATR для контекста алерта), `close_price_after` (цена через N дней — для дозаписи результатов), `ALGO_VERSION`, `FIB_LABELS`. Алерты/дайджест/статистика — всегда дневной таймфрейм

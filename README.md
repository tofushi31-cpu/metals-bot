...
Каждый алерт записывается с контекстом (RSI, ATR, версия алгоритма), а раз в сутки бот дописывает, куда цена ушла через 1/3/7 дней. Так копится собственная статистика отработки уровней — команда `/stats` показывает среднее движение цены после входа в зону по каждому инструменту и уровню. Скачанные цены кэшируются на `CACHE_TTL_MINUTES` минут (по умолчанию 10), чтобы дайджест и алерты не ходили в Yahoo за одним и тем же.
